import { GetUserProfile } from '@/shared/server-contracts/rest-schema';
import { Unauthorized } from '../../core/error-handling';
import { createProcedure } from '../../core/procedure';

const GROUPS: GetUserProfile['responses'][200]['groups'] = [
  {
    id: 0,
    key: 'basics',
    label: 'Basics',
...
export const getUserProfileQuestions = createProcedure<GetUserProfile>(
  async ({ db }) => {
    ...
    if (error) return new Unauthorized().json();
    ...
    // const { data, error } = await db.from('profile_questions').select('*');
...
//   (db: Supabase): Query['profileQuestions'] =>
//   async () => {
//     const { data, error } = await db.from('profile_questions').select('*');
...
//     return (data ??
//       []) as unknown as Database['public']['Tables']['profile_questions']['Row'][];
//   };
